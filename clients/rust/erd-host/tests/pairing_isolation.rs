...
fn run_isolated_subprocess(case: &str, test_name: &str, setup_fn: impl FnOnce(&std::path::Path)) {
    ...
    child
        ...
        .env("ERD_PAIRING_ISOLATION_CASE", case)
...
fn test_outbound_client_pairing_rejected_as_host_authorization() {
    if env::var("ERD_PAIRING_ISOLATION_CASE").as_deref() == Ok("outbound-rejected-and-isolation") {
        // Step 1: Functional trust isolation ahead of filename strings.
        // Client saves an outbound credential via open_default().
        let client_store = erd_app::PairingStore::open_default().unwrap();
        let client_key = vec![0x42; 32];
        let client_record = erd_app::PairingRecord {
        ...
        let disconnect_payload = erd_proto::ControlMessage::Disconnect.encode().unwrap();
        let mut disconnect_packet = erd_proto::PacketHeader::new(
            erd_proto::PacketType::Control,
            0,
            0,
            0,
        )
        ...
        let client_default_path = erd_app::PairingStore::default_path().unwrap();
        assert_eq!(
...
fn test_legacy_pairing_keys_not_auto_imported_by_host_and_migrated_by_client() {
    if env::var("ERD_PAIRING_ISOLATION_CASE").as_deref() == Ok("legacy-migration-and-host-isolation") {
    ...
    run_isolated_subprocess(
        ...
        |data_dir| {
            let ecliptic_dir = data_dir.join("EclipticRD");
            fs::create_dir_all(&ecliptic_dir).unwrap();
            let legacy_file = ecliptic_dir.join("pairing-keys.json");
