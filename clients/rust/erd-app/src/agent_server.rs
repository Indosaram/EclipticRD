...
mod tests {
    ...
    struct GatedBackend {
        screenshot: bool,
        events: std::sync::Mutex<Vec<erd_proto::InputEvent>>,
    ...
    async fn disconnect_releases_held_input_with_reset_transaction_before_answering() {
        ...
        assert_eq!(
            events,
            vec![
                erd_proto::InputEventType::LeftMouseDown,
                erd_proto::InputEventType::LeftMouseUp,
                erd_proto::InputEventType::Reset,
            ]
        );
        tokio::time::timeout(std::time::Duration::from_secs(5), worker)
            .await
    ...
    impl AgentServerBackend for FailingReleaseBackend {
        fn send_input_event(&self, _: erd_proto::InputEvent) -> Result<(), String> {
