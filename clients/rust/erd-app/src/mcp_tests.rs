...
struct FailingBackend(std::sync::Mutex<Vec<erd_proto::InputEventType>>);
...
fn partial_dispatch_resets() {
    ...
    assert!(backend
        ...
        .lock()
        .unwrap()
        .contains(&erd_proto::InputEventType::Reset));
}
...
fn handles_mcp_protocol_handshake_and_tool_calls() {
    ...
    assert_eq!(init["result"]["serverInfo"]["name"], "eclipticrd-mcp");
