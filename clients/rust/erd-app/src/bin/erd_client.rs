    ...
    if cli.agent_server.is_some() || cli.mcp {
        ...
        if let Some(port_opt) = cli.agent_server {
            ...
            let token = if let Some(ref t) = cli.agent_token {
                Some(t.clone())
            } else if let Ok(env_token) = std::env::var("ERD_AGENT_TOKEN") {
                Some(env_token)
                ...
                let gen = random_bytes
                    ...
                    .map(|b| format!("{:02x}", b))
                    .collect::<String>();
                eprintln!("[erd-client] Agent HTTP server auth token: {}", gen);
                Some(gen)
            };
            if token.is_some() {
                eprintln!("[erd-client] Agent HTTP server auth token enabled");
            }
