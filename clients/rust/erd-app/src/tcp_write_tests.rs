...
fn fixture() -> Fixture {
    ...
    let host = thread::spawn(move || {
        ...
        assert_eq!(
            ...
            Some(b"erd-p1.runtime-write".as_slice())
        );
...
fn input(kind: erd_proto::InputEventType) -> InputEvent {
    InputEvent {
        event_type: kind,
        x: 0.25,
        y: 0.75,
...
fn runtime_pressure_drains_ordered_controls_input_and_generated_pong() {
    ...
    pressurize(&session);
    let down = input(erd_proto::InputEventType::LeftMouseDown);
    let up = input(erd_proto::InputEventType::LeftMouseUp);
...
fn runtime_surfaces_callback_backpressure_instead_of_dropping_pong() {
    ...
    *session.tcp_write_wait.lock().unwrap() = Some(blocked_tx);
    ...
    session
        .send_packet(
            PacketType::Control,
            &vec![0; erd_proto::MAX_TCP_FRAME_SIZE - PacketHeader::SIZE],
        )
        .unwrap();
