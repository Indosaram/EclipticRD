...
use erd_net::{DatagramCipher, Direction, PskIdentity, TlsPskServer};
use erd_proto::{
...
async fn execute(scenario: Scenario, unwritable: bool) -> Option<Value> {
    ...
    let peer = tokio::spawn(async move {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let mut probe = [0; 1];
            let (_, address) = udp.recv_from(&mut probe).unwrap();
    ...
    let mut command = tokio::process::Command::new(env!("CARGO_BIN_EXE_erd-client"));
    command
        .args([
            "--host",
            "127.0.0.1",
    ...
    if matches!(scenario, Scenario::Empty) {
        ...
        assert_eq!(
            snapshot,
            &serde_json::to_value(erd_app::ReceiverSnapshot::default()).unwrap()
        );
