...
use erd_mobile::{
    erd_mobile_create, erd_mobile_destroy, erd_mobile_send_touch, AndroidAudioTrackPlayer,
    AndroidMediaCodecConfig, AndroidMediaCodecDecoder, ErdMobileStatus, IosAudioEnginePlayer,
...
fn unconnected_handle_send_touch_must_not_return_ok() {
    ...
    unsafe {
        let create_status = erd_mobile_create(host.as_ptr(), 19730, &mut handle);
        assert_eq!(create_status, ErdMobileStatus::Ok);
        ...
        assert_ne!(touch_status, ErdMobileStatus::Ok);
    }
}
...
fn create_with_empty_host_must_return_error() {
    ...
    let status = unsafe { erd_mobile_create(empty_host.as_ptr(), 19730, &mut handle) };

    // Then: Creation must fail.
    if status == ErdMobileStatus::Ok && !handle.is_null() {
        unsafe {
            let _ = erd_mobile_destroy(handle);
        }
    }
    assert_ne!(status, ErdMobileStatus::Ok);
}
...
fn create_failure_with_sentinel_must_reset_out_handle_to_null() {
    ...
    let status = unsafe { erd_mobile_create(empty_host.as_ptr(), 19730, &mut out_handle) };

    // Then: Creation fails and the slot is reset to null.
    if status == ErdMobileStatus::Ok && out_handle != sentinel && !out_handle.is_null() {
        unsafe {
            let _ = erd_mobile_destroy(out_handle);
        }
    }
    assert_ne!(status, ErdMobileStatus::Ok);
...
fn ios_audioengine_missing_native_backend_must_not_report_samples_rendered() {
    ...
    assert_eq!(
        start_res,
        Err(erd_mobile::ios::IosMediaError::BackendUnavailable)
    );
