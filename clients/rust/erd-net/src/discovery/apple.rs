...
const REGTYPE_C_STR: &CStr = match CStr::from_bytes_with_nul(b"_erd._tcp\0") {
    Ok(c) => c,
...
mod tests {
    ...
    fn construct_full_name_produces_valid_escaped_fullname() {
        let fullname = construct_full_name("My Host", "_erd._tcp.", "local.").unwrap();
        assert_eq!(fullname, "My Host._erd._tcp.local.");

        let dot_name = construct_full_name("host.1", "_erd._tcp", "local").unwrap();
        assert!(dot_name.ends_with("._erd._tcp.local."));
    }
