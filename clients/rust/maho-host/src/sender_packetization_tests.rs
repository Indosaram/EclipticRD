...
fn video_frame_exceeding_max_chunks_is_rejected() {
    ...
    let oversized_len = TARGET_VIDEO_CHUNK_BYTES * (maho_proto::MAX_CHUNKS_PER_FRAME as usize) + 1;
    let frame = VideoFrame {
    ...
    match result {
        Err(SessionError::Codec(maho_proto::CodecError::LengthLimit {
            field: "frame chunks",
            actual,
            max,
        })) => {
            assert_eq!(actual, (maho_proto::MAX_CHUNKS_PER_FRAME + 1) as usize);
            assert_eq!(max, maho_proto::MAX_CHUNKS_PER_FRAME as usize);
        }
