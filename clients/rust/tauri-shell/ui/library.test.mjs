...
const { normalizeHostKey, selectHosts, createFavoritesStorage, createLibrary } =
  createRequire(import.meta.url)('./library.js');
...
function memory(value = null) {
  ...
  return { values, getItem: name => values.get(name) ?? null,
    setItem: (name, data) => values.set(name, data) };
}
function deferred() {
  let resolve, reject;
...
function library(options = {}) {
  return createLibrary({ invoke: async () => hosts, storage: memory(), nativeAvailable: true, ...options });
}

test('classic script exposes the same four contract exports', () => {
  const context = { window: {} };
  runInNewContext(readFileSync(new URL('./library.js', import.meta.url), 'utf8'), context);
  expect(Object.keys(context.window.LibraryModel).sort()).toEqual([
    'createFavoritesStorage', 'createLibrary', 'normalizeHostKey', 'selectHosts',
  ]);
});
...
test('selection matches name, IP and OS; preserves order, identity and input', () => {
  ...
  for (const [query, expected] of [[' STUDIO ', [hosts[0], hosts[2]]],
    ['fe80::ab', [hosts[0]]], [' WINDOWS ', [hosts[1]]], ['absent', []]]) {
    expect(selectHosts(hosts, { query })).toEqual(expected);
  }
  expect(selectHosts(hosts, { availableOnly: true })).toEqual([hosts[0], hosts[2]]);
  expect(selectHosts(hosts, { query: 'studio', availableOnly: true,
    favoritesOnly: true, favoriteIps: [' FE80::AB ', '10.0.0.2', 'missing'] })).toEqual([hosts[0]]);
...
test('storage is a strict string-array boundary with normalized unique IP keys', () => {
  ...
  for (const invalid of ['{', '{}', 'null', '"ip"', '[1]', '["ip",null]', '[{"ip":"ip"}]']) {
    expect(() => createFavoritesStorage(memory(invalid)).load()).toThrow();
  }
  expect(() => adapter.save(['ip', 2])).toThrow();
  expect(() => createFavoritesStorage({ getItem() { throw new Error('denied'); } }).load()).toThrow();
  expect(() => createFavoritesStorage({ setItem() { throw new Error('quota'); } }).save(['ip'])).toThrow();
});
...
test('filter changes emit synchronously; All clears flags but not query; snapshots isolate arrays', async () => {
  const model = library();
  const events = [];
  const unsubscribe = model.subscribe(snapshot => events.push(snapshot));
  ...
  model.clearFilters();
  expect(events.at(-1)).toMatchObject({ query: 'Studio', availableOnly: false, favoritesOnly: false,
...
test('read/write storage failure is visible while session choices survive refresh', async () => {
  ...
  const model = library({ storage: { getItem: () => '["10.0.0.2"]',
...
test('latest requested refresh wins against stale success and rejection', async () => {
  for (const staleFails of [false, true]) {
    const requests = [];
    const model = library({ invoke: command => {
...
test('newest error cannot be erased by an older successful refresh', async () => {
  ...
  const older = model.refresh(), newer = model.refresh();
  requests[1].reject('latest failure'); await newer;
  requests[0].resolve(hosts); await older;
