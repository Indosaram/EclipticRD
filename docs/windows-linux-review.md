...
> 1.402 / 0.344136 / 0.714136 / 1.772로 **BT.601**이며, 행렬 자체는 호스트와 일치한다.
>
> 따라서 실제 불일치는 색공간(matrix)이 아니라 **레인지**였다. 셰이더가
> `(Y - 16/255) * (255/219)`와 `255/224` 스케일링으로 limited→full 확장을 적용해, 이미 full-range인 데이터의
> 검은색이 뭉개지고 하이라이트가 날아갔다. 수정은 셰이더에서 레인지 확장을 제거하는 것이며 행렬은 그대로 둔다.
> 회귀 방지를 위해 `src/lib/renderer.test.ts`에 호스트 정수 변환과 셰이더 수식을 왕복시키는 테스트를 추가했다.

- Windows `bgra_to_nv12`는 BT.601 limited-range를 사용한다. *(위 정정 참조: 실제로는 full-range)*
- Linux `encode_linux`의 수동 변환도 BT.601 limited-range를 사용한다. *(위 정정 참조)*
- 클라이언트(`tauri-shell`)는 BT.709로 렌더링한다. 호스트-클리언트 간 색공간 불일치가 있다. *(위 정정 참조: 클라이언트도 BT.601이며, 불일치는 레인지였다)*

**개선:** 프레임 헤더(현재 16바이트: width/height만)에 색공간과 레인지를 명시해 협상하면, 향후 인코더가 바뀌어도
클라이언트가 추측하지 않아도 된다.
