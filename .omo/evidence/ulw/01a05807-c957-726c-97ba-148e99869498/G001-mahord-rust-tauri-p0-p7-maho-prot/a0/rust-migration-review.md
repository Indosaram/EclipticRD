...
1. `clients/rust/maho-decode/src/lib.rs`: removed an unnecessary `as i32` cast from `AV_CODEC_FLAG2_FAST`.
2. `clients/rust/maho-host/src/capture_macos.rs`: removed the unused local declaration of `CGRequestScreenCaptureAccess`; permission requesting remains in the host executable onboarding path.
3. `clients/rust/maho-host/src/session.rs`: removed the unused capture timestamp field from the private `MediaEvent::Audio` variant and adjusted its private send/match sites.
...
#### MahoRDLogicTests: PASS

Command:

```sh
xcodebuild -scheme MahoRDLogicTests -destination 'platform=macOS' test
```
...
- `maho-proto` is a pure, I/O-free wire codec and framing layer.
- `maho-net` owns TLS-PSK, UDP-GCM, STUN, and signaling.
- `maho-decode` and `maho-render` isolate media decode and presentation primitives.
- `maho-app` composes client session, pairing, clipboard, input, ABR, reassembly, and latency behavior.
- `maho-host` separates platform capture/encode/input/clipboard implementations with `cfg` gates.
...
The main maintainability concern is concentration of orchestration: `maho-host/src/session.rs` is about 1,392 lines, and `maho-app/src/session.rs` is about 642 lines. Both combine state transitions, transport loops, media lifecycle, control handling, and persistence concerns. That raises change-coupling and makes platform/session behavior harder to review in isolation. The duplicated `tauri-shell/Cargo.toml` and `tauri-shell/src-tauri/Cargo.toml` manifests are another drift risk because they describe the same package with different relative paths.

### API hygiene

Strengths:

- `maho-proto` keeps implementation modules private and exports validated wire types through a compact `WireCodec` contract.
...
- `maho-app/src/lib.rs` uses broad glob re-exports from nearly every internal module, and `maho-host/src/lib.rs` exposes entire platform modules. This widens the semver surface and makes accidental API commitments likely.
...
- Documentation is stale: `clients/rust/README.md` says the default build does not require FFmpeg, while `maho-decode/Cargo.toml` currently enables `ffmpeg` by default.
...
- All modified `Sources/` files carry mtimes of **2026-08-31 22:32–23:12 KST** (verified via
  ...
  implementation phase* (`MahoCrypto.swift`, `MahoIdentity.swift`, `PairingPayloads.swift`,
...
- The substantive regression proof — Swift suites unaffected — passes directly:
  ...
  MahoRDLogicTests **TEST SUCCEEDED, 25 tests, 0 failures** (`/tmp/gate-xctest.log`),
